...
- **Multi-Hop:** It can traverse the `chunk_relations` table to perform 2-hop logical retrievals (e.g., finding Chunk A, reading its relation to Chunk B, and fetching Chunk B).
...
- **Provenance:** It generates a strict `provenance_map` that maps every sentence in the final answer back to a `source_chunk_id`.
